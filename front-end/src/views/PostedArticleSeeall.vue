...
const fetchNews = async (page = 1) => {
  try {
    const response = await fetch(`/api/user-posts?page=${page}&page_size=5`)
    const data = await response.json()
    if (data.error) {
      console.error(data.error)
      return
    }
    newsList.value = data.posts
    ...
    <div class="recent mt-2">
      ...
      >
        <div class="liked-posts-title">
          ...
          <small style="color: var(--grey)">You have posted {{ total_items }} articles</small>
        </div>
      </div>
